...
from PySide6.QtWidgets import (
    ...
    QFileDialog, QFrame, QTabBar, QShortcut
)
from PySide6.QtCore import Qt, Slot, Signal, QUrl, QSize, QObject
from PySide6.QtGui import QIcon, QTextCursor, QFont, QActionGroup, QDesktopServices, QPainter, QColor, QPixmap, QKeyEvent, QKeySequence
...
def _sanitize_filename(filename: str) -> str:
    ...
    Antes desta correção o cliente fazia apenas `.replace(" ", "_")`, o que
    permitia path traversal via nomes como `../../../../etc/cron.d/x`.
    """
    if not filename:
        return "arquivo.bin"
...
def _safe_temp_path(attachment_id: str, filename: str) -> str:
    ...
    Garante que o nome final seja apenas o basename sanitizado prefixado
    pelo ID do anexo — nunca uma rota de diretório arbitrária.
    """
...
class EmojiSelectorDialog(QDialog):
    ...
    def _on_emoji_clicked(self, char):
        if hasattr(self.line_edit, 'insertPlainText'):
            self.line_edit.insertPlainText(char)
        else:
...
class NotificationsDialog(QDialog):
    ...
    def _load_items(self):
        ...
        for req in self.controller.state.pending_friend_requests:
            ...
            # (QLabel suporta rich text por padrão, permitindo injeção HTML).
...
class MainWindow(QMainWindow):
    ...
    def _on_initial_connect(self, status: str):
        ...
        threading.Thread(
            target=lambda: self.controller.change_status("online"),
            daemon=True,
        ).start()
    ...
    def _on_tab_changed(self, index: int):
        if index == -1:
            return
        tab_name = self.chat_tabs.tabText(index).replace(" ⭐", "").strip()
    ...
    def _show_tab_context_menu(self, pos):
        ...
        tab_name = self.chat_tabs.tabText(tab_index).replace(" ⭐", "").strip()
    ...
    def _on_state_updated(self):
        ...
        for room_tab in self.controller.state.joined_rooms:
            ...
            for i in range(self.chat_tabs.count()):
                clean_title = self.chat_tabs.tabText(i).replace(" ⭐", "").strip()
        ...
        for i in range(self.chat_tabs.count() - 1, -1, -1):
            t_name = self.chat_tabs.tabText(i).replace(" ⭐", "").strip()
    ...
    def _load_room_members_async(self, room_name: str):
        ...
        def worker():
            try:
                ...
                # — executa na thread de background, mas é um set simples,
                # operação atômica em CPython.
    ...
    def _on_room_members_loaded(self, room_name: str, members: list):
        ...
        for m in members:
            ...
            if role == "owner":
                ...
                display_text = uname
            self.room_members_list.addItem(display_text)
    ...
    def _ensure_tab_exists(self, tab_name: str):
        ...
        chat_input.setPlaceholderText("Escreva sua mensagem... (Enter envia | Shift+Enter = nova linha)")
    ...
    def _show_notifications_popover(self):
        ...
        dlg.exec()
        for n in self.controller.state.notifications:
            n["read"] = True
        pending_invites_count = len(self.controller.state.pending_friend_requests)
        self.notify_btn.setText(f"🔔 ({pending_invites_count})")
    ...
    def _create_and_exec_context_menu(self, username, pos, parent_widget):
        ...
        if is_friend:
            ...
            remove_action.triggered.connect(lambda: self.controller.remove_friend(username))
        else:
    ...
    def _show_emoji_selector(self, line_edit):
        dlg = EmojiSelectorDialog(line_edit, self)
        dlg.exec()

    def _handle_attach_file(self, tab_name):
        filePath, _ = QFileDialog.getOpenFileName(self, "Selecionar Arquivo para Anexo")
        if filePath:
            file_size = os.path.getsize(filePath)
            if file_size > 10 * 1024 * 1024:
                QMessageBox.warning(self, "Erro de Anexo", "O arquivo excede o limite máximo de 10 MB.")
                return

            _, ext = os.path.splitext(filePath)
            blocked_exts = {".exe", ".bat", ".cmd", ".sh", ".com", ".scr", ".msi", ".dll", ".vbs", ".js", ".vbe", ".jse", ".wsf", ".wsh"}
            if ext.lower() in blocked_exts:
                QMessageBox.warning(self, "Erro de Anexo", "Tipo de arquivo não permitido (executável bloqueado).")
                return
